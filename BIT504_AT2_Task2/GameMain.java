...
public class GameMain extends JPanel implements ActionListener, MouseListener{ //ActionListener allows us to respond to action events
	...
	public GameMain() {   
		...
		// This JPanel fires a MouseEvent on MouseClicked so when the user clicks on the panel, an action event will be triggered        
	...
	public static void main(String[] args) {
		...
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
	         public void run() {
				...
				//Creates the new GameMain panel and adds it to the frame(i.e. window)
				frame.setContentPane(new GameMain());		
								
				//Sets the default close operation of the frame/window to exit_on_close
				frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);    
				
				frame.pack();             
				frame.setLocationRelativeTo(null);
				frame.setVisible(true);
				//Sets resizable frame to false so that the user can't resize the frame
	...
	public void paintComponent(Graphics g) {
		...
		if (currentState == GameState.Playing) {          
			...
			if (currentPlayer == Player.Cross) {   
				//Use the status bar to display the message in the red colour
